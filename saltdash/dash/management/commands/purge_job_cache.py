...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'days',
            type=int,
            help="Delete job cache items older than this many days")
        parser.add_argument(
            '--no-input',
            action='store_true',
            dest='no_input',
    ...
    def handle(self, *args, **options):
        days = options['days']
        if days < 1:
            raise CommandError('Not purging because less than 1 day specified. '
                               'Days = {}'.format(days))
        ...
        jobs = models.Job.objects.annotate(
            date=Func(F('jid'), Value('YYYYMMDDHH24MISSMS'),
        ...
        if not options['no_input']:
            response = input(
                "Are you sure you want to delete these items? [y/N]")
